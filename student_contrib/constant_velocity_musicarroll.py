...
x_true[0, 0] = x_true[0, 0] + w_true[0, 0] # truth at timestep 0
x_true[1, 0] = x_true[1, 0] + w_true[0, 0] # truth at timestep 0

for k in range(1, num_samples):
    x_true[:, k] = Phi.dot(x_true[:, k-1]) + w_true[:, k-1]

# Estimate:
x_hat = np.zeros((2, num_samples))
prior_est = np.zeros((2, num_samples))  # (x-)
# Initial state estimate:
x_hat[:, 0] = prior_est[:,0] = [0, 1]
...
for k in range(1, num_samples):
    ...
    # The current measurement:matplotlib.rcParams['axes.titlesize'] = 8
...
plt.plot(range(num_samples), z[0,:], 'r--o', label='Measurements')
plt.plot(range(num_samples), x_hat[0, :], 'd:k', label='Estimate')
plt.legend(loc='best')
plt.xlabel('Time')
plt.ylabel('Position (m)')
plt.title('Kalman Estimator: Non-accelerated 1D Motion with Vel Sensor\n'+ param_string)
...
plt.plot(range(num_samples), z[1,:], 'r--o', label='Measurements')
...
plt.title(f'Variance in Position ({q_string} Process Noise) and Vel Sensor\n'+param_string)
plt.xlabel('Time')
...
plt.figure(3)
plt.plot(new_time, collated)
plt.title(f'Variance in Velocity ({q_string} Process Noise) and Vel Sensor\n'+param_string)
plt.xlabel('Time')
